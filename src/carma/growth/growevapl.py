...
def growevapl(pc, growlg, evaplg,
    ...
    for ig in range(ngroup):
        ...
        # The inner `jnp.maximum(pc_val, 1e-50)` already guards the PPM
        # divide-by-tiny hazard. An additional relative-threshold mask
        # used to live here; removing it lets the distribution tail
        # propagate to ~1e-44 instead of being clipped ~20 decades
        # below the peak.
        for ibin in range(nbin - 1):
            dmdt_val = dmdt[ibin]
