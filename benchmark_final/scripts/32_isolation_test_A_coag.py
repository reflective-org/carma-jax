"""Test A — coagulation-only physics isolation across 3 solvers.

Turns OFF every process except binary particle-particle coagulation.
The aerosol distribution shrinks in number (large + small → larger)
while preserving total mass exactly.

Process toggles per solver:

  - Fortran: ``test_sulfate_realistic <scen> <out>  do_coag=1
              do_grow=0 dtime=<dt> nstep=48 fixed_h2so4=0 do_nuc=0``
    With do_grow=0, AddNucleation is skipped automatically; do_nuc=0
    is redundant but kept explicit.
  - Faithful JAX: ``cfg.do_coag=True, cfg.do_grow=False``; step built
              with ``do_homogeneous=False``.
  - Diffrax: ``make_rhs(shape, do_growth=False, do_homogeneous=False)``,
              ``coag=CoagBundle(...)`` from the operator-split wrapper
              landed in Phase 5 readiness work.

Sweep:
  - 2 atmospheres: strat39, trop96.
  - 5 timesteps:   dt ∈ {1, 10, 60, 300, 1800} s, fixed nsteps=48.

That's 30 runs total. Output goes to
``benchmark_final/outputs/iso_test_A/<atm>_dt<dt>/all_solvers.npz``.

Note: coagulation is rate-limited by the number density and the coag
kernel. For low-N seeds like scen 39 (M=0.26 µg/m³, GMD=751 nm → only
~1.2 #/cm³ total) the effect over 24 h is small. Diffrax uses
operator-split coag (one apply_coag call per outer step), so its
effective coag step is ``dtime`` — same as Fortran.
"""
...
def run_fortran(s, dtime, nstep, out_dir):
    """Fortran: do_coag=1, do_grow=0, do_nuc=0 (coag-only)."""
...
def _build_initial(s, cfg):
    ...
    return pc_per_bin, h2o_mmr, rhoa, p_cgs_val, T_K


def run_faithful_jax(s, dtime, nstep):
    """Faithful JAX coag-only: cfg.do_coag=True, cfg.do_grow=False, do_homogeneous=False."""
    import jax
...
def run_diffrax(s, dtime, nstep):
    """Diffrax coag-only: do_growth=False, do_homogeneous=False, coag=CoagBundle."""
    import jax
    jax.config.update("jax_enable_x64", True)
    import jax.numpy as jnp
    from jax_ensemble import _minimal_config, _compute_ppm_coefs, _refresh_env
    ...
    # Build a no-physics diffrax step (the ODE is trivially zero with
    # do_growth=False, do_homogeneous=False), but we still need a
    # diffeqsolve to drive 'time' forward. Even cleaner: skip diffrax
    # entirely and just apply coag operator-split per step.
    t_hist = np.zeros(nstep)
    gc_hist = np.zeros((nstep, NGAS))
    pc_hist = np.zeros((nstep, NBIN))
    t0_wall = time.perf_counter()
    for istep in range(nstep):
        ...
        # Coag-only: skip the diffeqsolve, just apply coag.
...
def main():
    ...
    for atm_key in args.atm:
        ...
        print(f"\n=== Test A (coag-only) — {atm_key}: scen {scen_idx} ===")
        for k, v in s.items():
            print(f"  {k}: {v}")
        for dt in args.dt:
            print(f"\n--- dt = {dt} s × {NSTEP} steps "
            ...
            bundle = {"scenario_info": np.array(json.dumps(
                dict(s, dt=dt, nstep=NSTEP, atm=atm_key, test="coag_only"),
            ))}
            ...
            if "diffrax" not in args.skip:
                print("  Diffrax (operator-split coag)...")
