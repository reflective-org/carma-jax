...
subroutine test_sulfate_realistic()
  ...
  integer                      :: unit_sched
  ! Time-series trajectory output for diagnostic plots.
  real(kind=f), allocatable    :: t_history(:)         ! (nstep,)
  real(kind=f), allocatable    :: gc_history(:,:)      ! (nstep, NGAS)
  real(kind=f), allocatable    :: pc_history(:,:)      ! (nstep, NBIN)
  character(len=512)           :: history_path
  integer                      :: unit_hist
  ...
  do istep = 1, nstep
    ...
    ! Record state after this outer step.
    t_history(istep)      = t(1)
    gc_history(istep, :)  = mmr_gas(1, :)
    pc_history(istep, :)  = mmr(1, 1, :)
  end do

  history_path = trim(output_path) // '.history.bin'
  open(newunit=unit_hist, file=trim(history_path), &
       access='stream', form='unformatted', &
       action='write', status='replace', iostat=ios)
  if (ios /= 0) then
    write(0, '(A, A)') 'cannot write history: ', trim(history_path)
    call exit(7)
  end if
  ! Layout (little-endian doubles, NSTEP-blocked):
  !   t_history(NSTEP)
  !   gc_history(NSTEP, NGAS)     column-major like Fortran
  !   pc_history(NSTEP, NBIN)
  write(unit_hist) t_history
  write(unit_hist) gc_history
  write(unit_hist) pc_history
  close(unit_hist)
