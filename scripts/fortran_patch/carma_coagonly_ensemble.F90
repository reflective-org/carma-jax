...
program carma_coagonly_ensemble
  implicit none
  call test_coagonly_ensemble()
end program

subroutine test_coagonly_ensemble()
  ...
  ! Coag-only test: keep AddGrowth/AddNucleation registrations so CARMA
  ! sets up the data structures consistently, but disable do_grow at
  ! initialization so microfast is skipped — only microslow (coag) runs.
  ...
  call CARMA_Initialize(carma, rc, do_grow=.false., do_coag=.true., &
...
end subroutine test_coagonly_ensemble
