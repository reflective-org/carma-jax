...
def main():
    ...
    nz_per_bin_for_plot = [
        np.where(x > 1e-16, x, 1e-16) for x in nz_per_bin
    ]
    bp = ax2.boxplot(
        [(x if x.size else np.array([1e-16])) for x in nz_per_bin_for_plot],
    ...
    ax2.set_title("Per-bin rel-err (box: cross-scenario P25/median/P75)")
