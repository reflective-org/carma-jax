"""Bench JAX freezaerl_koop2000 against the standalone Fortran kernel.

Same pattern as Phase 11.1/11.2.
Output: data/freezaerl_koop2000_bench.npz +
        plots/diff/phase11/freezaerl_koop2000_jax_vs_fortran.png
"""
import argparse
import math
import subprocess
...
def sample_scenarios(n, rng):
    ...
    Same range as Mohler (T < 240 gate) plus pressure: stratosphere and
    upper-troposphere range of 50 - 500 hPa = 5e4 - 5e5 dyne/cm² × 10
    (CGS) — actually CGS pressure is dyne/cm² = 10 × Pa, so 50 hPa =
    5e4 dyne/cm².
    """
    return dict(
        T          = rng.uniform(180.0,  240.0,  n),
        p_dyn      = 10.0 ** rng.uniform(np.log10(5e4), np.log10(5e5), n),
...
def run_fortran(binary, scen, r_bins, vol_bins, work_dir):
    ...
    with open(in_path, "w") as f:
        f.write(f"{scen['T']!r}  {scen['p_dyn']!r}  {scen['ssi']!r}  {scen['ssl']!r}  "
...
def main():
    ...
    if not args.binary.exists():
        print(f"ERROR: build with build_freezaerl_koop2000_standalone.sh first",
    ...
    np.savez_compressed(
        ROOT / "data" / "freezaerl_koop2000_bench.npz",
    ...
    ax.set_title(f"Phase 11.3: freezaerl_koop2000 — JAX vs Fortran "
    ...
    out_path = out_dir / "freezaerl_koop2000_jax_vs_fortran.png"
    plt.tight_layout()
