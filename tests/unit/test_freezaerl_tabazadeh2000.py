"""Unit tests for freezaerl_tabazadeh2000 (Phase 11.2).

Bench against the standalone gfortran-compiled kernel
(scripts/fortran_patch/freezaerl_tabazadeh2000_standalone.F90, built
via build_freezaerl_tabazadeh2000_standalone.sh).
...
FORTRAN_BIN = (REPO.parent / "original-carma" / "CARMA"
               / "build_standalone" / "freezaerl_tabazadeh2000_standalone")
...
def _run_fortran(scen, r_bins, vol_bins, work):
    ...
    with open(in_path, "w") as f:
        f.write(f"{scen['T']!r}  {scen['ssi']!r}  {scen['ssl']!r}  "
                f"{scen['akelvin']!r}  {scen['rhosol']!r}  {scen['gwtmol']!r}  "
...
_FORTRAN_AVAILABLE = FORTRAN_BIN.exists()
_skip_no_fortran = pytest.mark.skipif(
    not _FORTRAN_AVAILABLE,
    reason=f"Fortran bench binary not built. Run "
           f"`bash scripts/fortran_patch/build_freezaerl_tabazadeh2000_standalone.sh`.")


SCENARIOS = [
    # (label, T, ssi, ssl, akelvin, rhosol, gwtmol, pconmax)
    ("cirrus_typical",  220.0, 0.50, -0.40, 1.5e-7, 1.78, 18.016, 1e3),
    ("very_cold_high",  200.0, 0.80, -0.20, 1.4e-7, 1.78, 18.016, 1e4),
    ("warm_active",     250.0, 0.50, -0.40, 1.5e-7, 1.78, 18.016, 1e3),  # no T-gate
    ("low_aw_regime",   220.0, 0.50, -0.99, 1.5e-7, 1.78, 18.016, 1e3),
    ("high_aw_regime",  220.0, 0.50, -0.05, 1.5e-7, 1.78, 18.016, 1e3),
    # gates
    ("low_ssi_skip",    220.0, 0.10, -0.40, 1.5e-7, 1.78, 18.016, 1e3),
    ("no_particles",    220.0, 0.50, -0.40, 1.5e-7, 1.78, 18.016, 1e-50),
]


@_skip_no_fortran
@pytest.mark.parametrize("label,T,ssi,ssl,akelvin,rhosol,gwtmol,pconmax",
                          SCENARIOS)
def test_freezaerl_tabazadeh2000_matches_fortran(
        label, T, ssi, ssl, akelvin, rhosol, gwtmol, pconmax, tmp_path):
    ...
    scen = dict(T=T, ssi=ssi, ssl=ssl, akelvin=akelvin,
                rhosol=rhosol, gwtmol=gwtmol, pconmax=pconmax)
    F = _run_fortran(scen, r_bins, vol_bins, tmp_path)
    J = _run_jax(scen, r_bins, vol_bins)
    # 1e-9 is comfortable headroom over the bench-observed max 2.4e-10
    # (Myhre 1998 wtfrac^10 polynomial + surface-tension polynomials
    # accumulate more FMA drift than Möhler did).
    if np.any(F > 0):
        ...
        np.testing.assert_array_equal(F, J)   # both zero


def test_freezaerl_tabazadeh2000_jit_equivalent():
    r_bins, vol_bins = _bin_grid()
    args = dict(
    ...
    out      = freezaerl_tabazadeh2000(**args)
    out_jit  = jax.jit(freezaerl_tabazadeh2000)(**args)
    np.testing.assert_allclose(np.asarray(out), np.asarray(out_jit),
                                rtol=1e-12, atol=0.0)


def test_freezaerl_tabazadeh2000_gates():
    r_bins, vol_bins = _bin_grid()
    base = dict(
        akelvin_val=jnp.float64(1.5e-7),
        r_bins=jnp.asarray(r_bins), vol_bins=jnp.asarray(vol_bins),
    ...
    active = freezaerl_tabazadeh2000(
        t_val=jnp.float64(220.0), supsati_val=jnp.float64(0.5),
        supsatl_val=jnp.float64(-0.4), pconmax_val=jnp.float64(1e3), **base)
    assert bool(jnp.any(active > 0))

    for kwargs in (
        dict(t_val=jnp.float64(220.0), supsati_val=jnp.float64(0.1),  # ssi < 0.3
             supsatl_val=jnp.float64(-0.4), pconmax_val=jnp.float64(1e3)),
        dict(t_val=jnp.float64(220.0), supsati_val=jnp.float64(0.5),
             supsatl_val=jnp.float64(-0.4),
             pconmax_val=jnp.float64(1e-50)),                          # pconmax < FEW_PC
    ):
        out = freezaerl_tabazadeh2000(**kwargs, **base)
